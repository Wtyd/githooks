...
| ID      | Área     | Test                                                 | Comando                                                                               | Salida esperada                                                                                                                                     | Exit | Config            |
| ------- | -------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ----------------- |
| V32-018 | fast+fmt | `--fast --format=json` con skipped: stdout limpio    | `GH flow qa --fast --format=json --config=githooks-fast.php 1>/tmp/o 2>/tmp/e` (sin staged) | `/tmp/o` es JSON parseable (empieza por `{`). `/tmp/e` contiene mensajes de progreso / `⏩ was skipped`. Nunca se mezclan                              | 0/1  | githooks-fast.php |
| V32-019 | fast+fmt | `--fast --format=junit` con skipped: stdout limpio   | `GH flow qa --fast --format=junit --config=githooks-fast.php 1>/tmp/o 2>/tmp/e` (sin staged) | `/tmp/o` empieza por `<?xml`. `/tmp/e` tiene progreso. Sin contaminación cruzada                                                                     | 0/1  | githooks-fast.php |
...
| v3.2      | 23      | cores override (phpcs/paratest/phpstan/custom/mixed), conflict warning, `--output=PATH` en los 4 formatos, stdout por defecto, `--stdout` ignorado, JUnit `<skipped>`, GitLab annotations, conf:check truncation, live streaming, dashboard TTY, fast+estructurado stdout limpio, executionMode real en JSON, Code Climate + SARIF paths relativos, fail-fast listado en JSON |
| **Total** | **189** |                                                                                                                                                                                                                                      |
