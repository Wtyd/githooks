...
| V32-026 | stderr | Progreso stderr forzado con `--show-progress`        | `GH flow qa --format=json --show-progress --config=githooks-v3.php 2>/tmp/e`                         | `/tmp/e` contiene `OK/KO jobname [n/m]` + `Done. N/N`, aunque no haya TTY (CI-friendly). `-v` por sí solo ya NO fuerza progreso                      | 0/1  | githooks-v3.php |
