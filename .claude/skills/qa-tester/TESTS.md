...
| v3.2      | 21      | cores override (phpcs/paratest/phpstan/custom/mixed), conflict warning, `--output=PATH` en los 4 formatos, stdout por defecto, `--stdout` ignorado, JUnit `<skipped>`, GitLab annotations, conf:check truncation, live streaming, dashboard TTY, fast+estructurado stdout limpio, executionMode real en JSON, Code Climate paths relativos |
| **Total** | **187** |                                                                                                                                                                                                                                      |
