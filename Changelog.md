...
### New Commands
- `githooks flow <name>` — Run a flow by name. Supports `--fail-fast`, `--processes=N`, `--exclude-jobs=a,b`, `--only-jobs=a,b`, `--dry-run`, `--format=json|junit`, `--fast`, `--monitor`, `--config=path`.
- `githooks job <name>` — Run a single job by name. Supports `--dry-run`, `--format=json|junit`, `--fast`, `--config=path`.
...
- `githooks cache:clear [jobs...]` — Clear cache files generated by QA tools. Without arguments, clears all caches; with job names, clears only those. Supports `--config=path`.
...
- **`cache:clear` command**: Delete cache files generated by QA tools (PHPStan tmpDir, PHPMD `.phpmd.cache`, PHPCS `.phpcs.cache`, Psalm `.psalm/cache/`, PHPUnit `.phpunit.result.cache`). Accepts job names to clear selectively.
