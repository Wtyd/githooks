...
- **TTY-aware progress with verbose override**: the stderr progress handler only emits when stderr is a TTY, so `flow|job --format=json | jq ...` works off pipes, CI and agents **without `2>/dev/null`**. Pass `-v` (verbose) to force progress even off a TTY — useful for long-running CI pipelines. `--dry-run` never emits progress.
