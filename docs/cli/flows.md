...
The same flag set as [`flow`](flow.md): `--fail-fast`, `--processes`, `--exclude-jobs`, `--only-jobs`, `--format`, `--output`, `--report-*`, `--no-reports`, `--fast`, `--fast-branch`, `--fast-dirty`, `--files`, `--files-from`, `--exclude-pattern`, `--monitor`, `--save-history`, `--diag`, `--stats`, `--stats-sort`, `--no-ci`, `--show-progress`, `--config`, `--dry-run`. Only the positional argument changes (variadic instead of single). `--diag` prints a [runtime diagnostics block](../how-to/output-formats.md#runtime-diagnostics-and-absolute-timestamps) before the run (auto-on in CI); `--stats-sort=exec\|name\|type` orders the `--stats` table.
