...
The trailing `"$@"` forwards the arguments Git passes to the hook (the message-file path for
`commit-msg`, for instance). When the hooks are installed with `--config=<file>`, the script also
carries that file as `--config`, so every trigger runs the configuration the hooks were installed
from — see [`githooks hook`](../cli/hook.md).
