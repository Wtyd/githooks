...
class ConfigurationFileValidator
{
     ...
     *
     * @return Errors POPO that contains an array of errors and one of warnings.
     */
    public function __invoke(array $configurationFile): Errors
    {
        $Errors = new Errors();
        ...
        return $Errors->setOptionsErrors($optionsErrorsAndWarnings[0])
     ...
     */
    public function checkValidKeys(array $array, array $validKeys): array
    {
        $warnings = [];
     ...
     * @return array
     */
    public function checkOptionsKey(array $configurationFile): array
    {

        $errors = [];
        $warnings = [];
        if (!array_key_exists(ConfigurationFile::OPTIONS, $configurationFile)) {
            return [$errors, $warnings];
        }

        if (empty($configurationFile[ConfigurationFile::OPTIONS])) {
            $warnings[] = 'The tag \'' . ConfigurationFile::OPTIONS . '\' is empty';
        } else {
            $warnings = $this->checkValidKeys($configurationFile[ConfigurationFile::OPTIONS], ConfigurationFile::OPTIONS_KEY);

            $validItems = $this->extractValidItems($configurationFile[ConfigurationFile::OPTIONS]);
    ...
    protected function extractValidItems(array $options): array
    {
        ...
        foreach ($options as $key => $value) {
            if (in_array($key, ConfigurationFile::OPTIONS_KEY)) {
    ...
    protected function checkToolsKey(array $configurationFile): array
    {
        ...
        if (!isset($configurationFile[ConfigurationFile::TOOLS]) || empty($configurationFile[ConfigurationFile::TOOLS])) {
            $errors[] = "The key 'Tools' must exists.";
            return [$errors, $warnings];
        }

        foreach ($configurationFile[ConfigurationFile::TOOLS] as $tool) {
            ...
            if (!array_key_exists($tool, ToolAbstract::SUPPORTED_TOOLS)) {
                ...
                $errors = array_merge($errors, $toolErrors[ConfigurationFile::ERRORS]);
                $warnings = array_merge($warnings, $toolErrors[ConfigurationFile::WARNINGS]);
            }
        }
    ...
    protected function checkConfiguration(array $configurationArguments, array $expectedValues, string $tool): array
    {
        ...
        return [ConfigurationFile::ERRORS => $errors, ConfigurationFile::WARNINGS => $warnings];
    }
}
